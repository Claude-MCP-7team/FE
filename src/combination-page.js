...
function renderCombination(combination) {
  return `<article class="card"><h3>${combination.rank}순위 · ${money(combination.total_krw)}${combination.total_is_estimated ? ' · 금액 미확정' : ''}</h3><ul>${combination.members.map(member => `<li><strong>${escape(member.title)}</strong><small> ${money(member.estimated_total_krw)}${member.amount_estimated ? ' (금액 미확정)' : ''}</small></li>`).join('')}</ul>${combination.excluded.length ? `<details><summary>제외된 정책 ${combination.excluded.length}개</summary>${combination.excluded.map(item => `<div class="conflict"><strong>${escape(item.title)}</strong> ${chip(item.confidence)}<p>${escape(item.conflicts_with_title)}와 함께 받을 수 없어 제외됐어요.</p><blockquote>${escape(item.source_quote)}${item.source_url ? `<br><a href="${escape(item.source_url)}" target="_blank" rel="noreferrer">근거 원문</a>` : ''}</blockquote></div>`).join('')}</details>` : ''}</article>`;
}
