...
function profile() {
  const fields = { birth_date: '생년월일', region: '거주지역', residence_start_date: '연속 거주 시작일', education: '학력', employment_status: '취업 상태', income: '개인 월 소득 (원)', household_income: '가구 월 소득 (원)', marital_status: '혼인 상태', policy_history: '기존 정책 참여 이력' };
  return heading('내 조건 입력', 'M0 입력 화면 구조입니다. 입력·검증·저장 기능은 M1에서 연결합니다.') + `<section class="card"><div class="fields">${Object.entries(fields).map(([key, label]) => `<label>${label}<input disabled value="${escape(data.profile[key] ?? '모름 / 미입력')}" aria-label="${label}"></label>`).join('')}</div><div class="actions">${link('analysis', '분석 시작 화면 예시')}</div></section>`;
}
function questions() {
