...
export function renderJudgementDashboard(view, { onDetail = id => `#/policies/${encodeURIComponent(id)}` } = {}) {
  ...
  return `<div class="hero"><span class="eyebrow">실제 판정 결과</span><h1>내 조건에 맞는 정책</h1><p class="muted">${escape(view.disclaimer)}</p></div><div class="summary judgement-summary"><article>신청 가능<strong>${summary.eligible}<small>개</small></strong></article><article>조건 미충족<strong>${summary.ineligible}<small>개</small></strong></article><article>추가 확인<strong>${summary.needs_info}<small>개</small></strong></article>${summary.future_eligible !== undefined ? `<article>향후 신청 가능<strong>${summary.future_eligible}<small>개</small></strong></article>` : ''}</div><div class="grid">${view.results.map(result => {
    const [label, style] = result.future_eligible_from ? ['향후 신청 가능', 'FUTURE_PASS'] : verdicts[result.verdict];
...
export function renderJudgementDetail(result) {
  ...
  return `<a class="button" href="#/results">← 정책 결과</a><div class="hero"><span class="eyebrow">정책 판정 상세</span><h1>${escape(result.policy_id)}</h1><p>${escape(result.explanation ?? '')}</p></div><section class="card"><p><span class="badge ${style}">${label}</span>${result.future_eligible_from ? ` <span class="muted">가능 시점 ${escape(result.future_eligible_from)}</span>` : ''} <span class="badge ${result.confidence === 'CONFIRMED' ? 'PASS' : 'UNKNOWN'}">${escape(result.confidence)}</span></p>${contact}<h2>조건별 판정과 원문 근거</h2>${conditions || '<p>조건 상세가 없습니다.</p>'}</section>`;
}
